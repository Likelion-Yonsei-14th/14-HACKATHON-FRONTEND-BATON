...
export const mockBatons: Baton[] = [
  {
    ...
    completedAt: null, // 결과 미확인 상태 (Home에 배지로 표시)
  },
  {
...
export const mockClassifications: Record<string, Classification> = {
  ...
  'baton-pending-ambiguous-1': {
    ...
    replyMessageId: 'msg-3-reply',
    selectedBranchId: null,
    // 스키마에 후보 분기 저장 컬럼이 없어 목데이터에서 임의로 2개 지정함 (docs/enum-proposals.md 참고)
    candidateBranchIds: ['baton-pending-ambiguous-1-branch-a', 'baton-pending-ambiguous-1-branch-b'],
    confidence: 0.51,
    isAmbiguous: true,
    containsNewQuestion: false,
...
export const mockExecutions: Record<string, Execution> = {
  'baton-result-1': {
    id: 'exec-result-1',
    batonId: 'baton-result-1',
    branchId: 'baton-result-1-branch-a',

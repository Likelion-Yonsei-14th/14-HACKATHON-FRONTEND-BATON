  ...
  return (
    <AppShell>
      <div className="font-suit flex items-center gap-2 text-sm">
        <Link className="text-muted hover:text-body" to="/home">
          바통 홈으로
        </Link>
        <span className="text-muted">›</span>
        <span className="font-medium text-strong">보류 응답 처리</span>
      </div>

      <Panel className="mt-6 max-w-3xl bg-warn-soft shadow-none">
      ...
      {replyMessage && (
        <Panel className="mt-2 max-w-3xl">
          <p className="text-sm font-medium text-strong">원문</p>
          <p className="mt-2 text-sm text-muted">{replyMessage.content}</p>
          {replyMessage.translatedContent && (
            <>
              <p className="mt-4 text-sm font-medium text-strong">번역 (한국어)</p>
      ...
      {useManualPath ? (
        <div className="mt-6 max-w-3xl">
          <h2 className="font-suit text-base font-medium text-strong">상대의 새 질문에 직접 응답 작성</h2>
          ...
          <textarea
            className="font-suit mt-3 w-full rounded-field bg-chip p-4 text-sm text-body shadow-hairline outline-none"
        ...
        <div className="mt-6 max-w-3xl">
          <h2 className="font-suit text-base font-medium text-strong">준비된 응답 분기</h2>
          ...
          <div className="mt-3 flex flex-col gap-2">
            {candidateBranches.map((branch) => (
              <label
                className={`flex cursor-pointer items-start gap-3 rounded-card p-4 ${
                  selectedBranchId === branch.id ? 'bg-card-raised shadow-card' : 'bg-card shadow-none'
                }`}
                key={branch.id}
              >
                <input
                ...
                />
                <span>
                  <strong className="text-strong">{branch.name}</strong>
      ...
      <Panel className="mt-6 max-w-3xl">
        <p className="font-medium text-strong">발송 전 확인</p>
        ...
        <div className="mt-4 flex justify-end gap-3">
          <Link className="font-suit self-center text-sm text-muted hover:text-body" to="/home">
            취소
          </Link>
